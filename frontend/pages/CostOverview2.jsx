    ...
    <>
      ...
      >
        <td className="w-10 px-4 py-3 text-center">
          <svg
            className={`h-4 w-4 text-gray-400 transition-transform ${isExpanded ? "rotate-90 text-primary" : ""}`}
            fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
          >
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
          </svg>
        </td>
        <td className={`whitespace-nowrap px-4 py-3 font-mono text-xs ${isExpanded ? "font-bold text-primary" : "text-gray-500"}`}>
          {row.session_id}
        </td>
      ...
      {isExpanded && (
        <tr>
          <td colSpan={7} className="border-b border-gray-100 bg-primary-soft/40 dark:border-gray-800 dark:bg-primary/20">
...
export default function CostOverview2({ params }) {
  ...
  // Handle incoming params (drill-down from CostAnalysis)
  useEffect(() => {
    if (params?.status) {
      setFilters(f => ({ ...f, statuses: [params.status] }));
    }
  }, [params]);
  ...
  }, [effectiveTimeBounds, page, sortKey, sortOrder, filters.agents, filters.users, filters.gateways, filters.models, filters.statuses, pageSize]);

  useEffect(() => { load(); }, [load]);
  useEffect(() => { setPage(1); }, [filters.timePreset, filters.agents, filters.users, filters.gateways, filters.models, filters.statuses]);
    ...
    <div className="space-y-4">
      ...
      <div className="app-card overflow-hidden border border-gray-100 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-4 py-3 dark:border-gray-800 sm:px-6">
          <h2 className="text-sm font-bold text-gray-900 dark:text-gray-100">{intl.get("costOverview2.title")}</h2>
          <div className="flex flex-wrap items-center gap-2">
            <MultiSelectFilter label="Agent" options={agentOptions} value={filters.agents} onChange={handleMultiChange("agents")} placeholder={intl.get("costOverview2.allAgent")} />
            ...
            <MultiSelectFilter label="Gateway" options={gatewayOptions} value={filters.gateways} onChange={handleMultiChange("gateways")} placeholder={intl.get("costOverview2.allGateway")} />
              ...
              <table className="w-full min-w-[800px] border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50 dark:border-gray-800 dark:bg-gray-900/50">
                    ...
                    <SortableTableTh label={intl.get("costOverview2.sessionId")} columnKey="session_id" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} />
                    ...
                    <SortableTableTh label={intl.get("costOverview2.agentUser")} columnKey="agentName" sortKey={sortKey} sortOrder={sortOrder} onSort={handleSort} />
